document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function getOrCreateErrorElement(field) {
      const wrapper = field.closest('.form-group, .field-wrapper, [data-field]') || field.parentElement;
    ...
    function setFieldError(field, message) {
      ...
      const errorEl = getOrCreateErrorElement(field);
    ...
    function validateField(field) {
      ...
      if (value.trim() !== '') {
        ...
        if (minLength && !validators.minLength(value, minLength)) {
          setFieldError(field, errorMessages.minLength(minLength));
          return false;
        }
      }
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      if (isFormValid) {
        ...
        const firstError = contactForm.querySelector('.' + FIELD_ERROR_CLASS + ' input, .' + FIELD_ERROR_CLASS + ' textarea, .' + FIELD_ERROR_CLASS + ' select');
        if (firstError) {
          firstError.focus();
        }
      }
    ...
    contactForm.addEventListener('input', function (e) {
      const field = e.target;
      const wrapper = field.closest('.form-group, .field-wrapper, [data-field]') || field.parentElement;
      if (wrapper && wrapper.classList.contains(FIELD_ERROR_CLASS)) {
        validateField(field);
      }
    });
  }
  ...
  const OBSERVE_CLASS = 'fade-in';
  const STAGGER_ATTRIBUTE = 'data-stagger';

  function applyStaggerDelay(elements) {
    elements.forEach(function (el, index) {
      if (!el.style.transitionDelay && !el.getAttribute('data-delay')) {
  ...
  if ('IntersectionObserver' in window) {
    ...
    const scrollObserver = new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting) {
          ...
          const staggerParent = el.querySelector('[' + STAGGER_ATTRIBUTE + ']');
          if (staggerParent) {
            const children = Array.from(staggerParent.children);
            applyStaggerDelay(children);
          }

          if (el.hasAttribute(STAGGER_ATTRIBUTE)) {
            const siblings = Array.from(el.parentElement.querySelectorAll('.' + OBSERVE_CLASS));
            applyStaggerDelay(siblings);
          }
    ...
    function observeAnimationTargets() {
      const targets = document.querySelectorAll('.' + OBSERVE_CLASS + ':not(.' + ANIMATION_CLASS + ')');
      targets.forEach(function (target) {
        const customDelay = target.getAttribute('data-delay');
        if (customDelay) {
          target.style.transitionDelay = customDelay + 'ms';
        }
    ...
    const mutationObserver = new MutationObserver(function (mutations) {
      mutations.forEach(function (mutation) {
        mutation.addedNodes.forEach(function (node) {
          if (node.nodeType === 1) {
            if (node.classList && node.classList.contains(OBSERVE_CLASS) && !node.classList.contains(ANIMATION_CLASS)) {
              scrollObserver.observe(node);
            }
            const nestedTargets = node.querySelectorAll && node.querySelectorAll('.' + OBSERVE_CLASS + ':not(.' + ANIMATION_CLASS + ')');
            if (nestedTargets) {
              nestedTargets.forEach(function (nested) {
                scrollObserver.observe(nested);
              });
            }
          }
        });
    ...
    const fallbackTargets = document.querySelectorAll('.' + OBSERVE_CLASS);
  ...
  (function applyRTLSupport() {
    ...
    const isRTL = htmlEl.getAttribute('dir') === 'rtl' || getComputedStyle(htmlEl).direction === 'rtl';
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "PopZone by Evyatar משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'bc0a8616-40f4-4fac-84b0-695471516f45',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'bc0a8616-40f4-4fac-84b0-695471516f45';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'bc0a8616-40f4-4fac-84b0-695471516f45';
